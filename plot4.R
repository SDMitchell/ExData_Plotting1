...
xresDefault <- 480
yresDefault <- 480
outputFilenameDefault <- "plot4.png"
...
createPlot <- function(inputData) {
	...
	# Upper left
	plot(y=inputData$Global_active_power, x=inputData$Timestamp, type="l", col="black", xlab="", ylab="Global Active Power", main="")

	# Upper right
	plot(y=inputData$Voltage, x=inputData$Timestamp, type="l", col="black", xlab="datetime", ylab="Voltage", main="")

	# Lower left
	plot(x=inputData$Timestamp, y=inputData$Sub_metering_1, col="black", type="l", xlab="", ylab="Energy sub metering", main="")
	points(x=inputData$Timestamp, y=inputData$Sub_metering_2, col="red", type="l")
	points(x=inputData$Timestamp, y=inputData$Sub_metering_3, col="blue", type="l")
	legend(x="topright", legend=c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), col=c("black", "red", "blue")
		   , lty=c(1, 1, 1), bty="n")

	# Lower right
	plot(y=inputData$Global_reactive_power, x=inputData$Timestamp, type="l", col="black", xlab="datetime", ylab="Global_reactive_power", main="")
}

##
## The driver code to create a PNG file out of the default data set.
##
png(filename=outputFilenameDefault, width=xresDefault, height=yresDefault, type="cairo")
